...
import { cellHash, CHUNK_SIZE, connectedWallSprite, getMapFrame, LABEL_FONTS, LABEL_SIZES, LINE_STYLES, OBJECT_ATLAS_COLUMNS, OBJECTS, resolveLabelColor, TERRAIN } from '../constants.js'
...
function drawObjects(){clear(roots.objects);if(!layerVisible('objects'))return;for(const savedItem of props.bundle.objects||[]){const item=props.liveObject?.id===savedItem.id?props.liveObject:savedItem;if(item.deletedAt)continue;const asset=objectById[item.assetId],frame=objectFrames[asset?.sprite];if(!asset||!frame)continue;const scale=item.scale||1,sprite=new Sprite(frame);sprite.anchor.set(.5);sprite.position.set((item.x??0)+.5,(item.y??0)+.5);sprite.width=asset.width*scale;sprite.height=asset.height*scale;sprite.rotation=(item.rotation||0)*Math.PI/180;sprite.zIndex=item.zIndex||0;roots.objects.addChild(sprite)}roots.objects.sortableChildren=true;roots.objects.sortChildren()}
...
function drawSelection(){clear(roots.selection);if(props.selection?.kind!=='object')return;const item=props.liveObject||props.bundle.objects?.find((entry)=>entry.id===props.selection.id);if(!item||item.deletedAt)return;const asset=objectById[item.assetId]||{width:1,height:1},scale=item.scale||1,box=new Graphics().rect(-asset.width*scale/2,-asset.height*scale/2,asset.width*scale,asset.height*scale).stroke({color:canvasTheme().selection,width:Math.max(.035,2/props.scale),alpha:.96});box.position.set(item.x+.5,item.y+.5);box.rotation=(item.rotation||0)*Math.PI/180;roots.selection.addChild(box)}
function drawTransient(){
  ...
  if(props.activeTool==='object'&&props.hoverPoint){const asset=objectById[props.selectedObject],frame=objectFrames[asset?.sprite];if(asset&&frame){const sprite=new Sprite(frame);sprite.anchor.set(.5);sprite.position.set(props.hoverPoint.x,props.hoverPoint.y);sprite.width=asset.width;sprite.height=asset.height;sprite.alpha=.72;roots.overlayPreview.addChild(sprite)}}
}
...
function updateFrameBoundary(){const frame=getMapFrame(props.bundle.project),width=frame.width*props.scale,height=frame.height*props.scale,left=host.value.clientWidth/2+(frame.x-props.cameraX)*props.scale,top=host.value.clientHeight/2+(frame.y-props.cameraY)*props.scale,right=host.value.clientWidth-left-width,bottom=host.value.clientHeight-top-height;host.value.style.setProperty('--frame-left',`${left}px`);host.value.style.setProperty('--frame-top',`${top}px`);host.value.style.setProperty('--frame-width',`${width}px`);host.value.style.setProperty('--frame-height',`${height}px`);host.value.style.setProperty('--frame-clip-left',`${Math.max(0,left)}px`);host.value.style.setProperty('--frame-clip-top',`${Math.max(0,top)}px`);host.value.style.setProperty('--frame-clip-right',`${Math.max(0,right)}px`);host.value.style.setProperty('--frame-clip-bottom',`${Math.max(0,bottom)}px`)}
function updateCamera(){if(!ready||!host.value)return;roots.world.position.set(host.value.clientWidth/2-props.cameraX*props.scale,host.value.clientHeight/2-props.cameraY*props.scale);roots.world.scale.set(props.scale);updateFrameBoundary();drawTerrainStrokes();drawLabels();drawGrid();drawSelection();drawTransient();requestRender()}
async function setup(){app=new Application();await app.init({resizeTo:host.value,background:BACKGROUND,antialias:true,autoDensity:true,resolution:Math.min(2,devicePixelRatio||1),preference:'webgl'});if(disposed)return;app.ticker.stop();app.canvas.className='map-scene-canvas';host.value.appendChild(app.canvas);roots.world=new Container();for(const name of ['legacy','strokes','terrainPreview','grid','structures','structurePreview','objects','labels','selection','overlayPreview'])roots[name]=new Container();roots.world.addChild(roots.legacy,roots.strokes,roots.terrainPreview,roots.grid,roots.structures,roots.structurePreview,roots.objects,roots.labels,roots.selection,roots.overlayPreview);app.stage.addChild(roots.world);await Promise.all(LABEL_FONTS.filter((font)=>!font.legacy).map((font)=>document.fonts.load(`16px "${font.family}"`)));const[sources,wallAtlas,objectAtlas,...lineAtlases]=await Promise.all([loadTerrainMaterialSources(requiredMaterialIds()),Assets.load('/assets/terrain/connected-walls-atlas.webp'),Assets.load('/assets/objects/story-shack-object-atlas.webp'),...LINE_STYLES.map((style)=>Assets.load(style.asset))]);if(disposed)return;materialSources=sources;materials=Object.fromEntries(Object.entries(sources).map(([id,source])=>{const texture=Texture.from(source);texture.source.style.addressMode='repeat';return[id,texture]}));const frames=(atlas,columns=4,count=columns*columns)=>{const width=atlas.width/columns,height=atlas.height/columns;return Array.from({length:count},(_,index)=>new Texture({source:atlas.source,frame:new Rectangle((index%columns)*width,Math.floor(index/columns)*height,width,height)}))},stripFrames=(atlas)=>{const count=12,width=atlas.width/count;return Array.from({length:count},(_,index)=>new Texture({source:atlas.source,frame:new Rectangle(index*width,0,width,atlas.height)}))};wallFrames=frames(wallAtlas);objectFrames=frames(objectAtlas,OBJECT_ATLAS_COLUMNS,OBJECTS.length);lineFrames=Object.fromEntries(LINE_STYLES.map((style,index)=>[style.id,stripFrames(lineAtlases[index])]));ready=true;syncModel();updateCamera();resizeObserver=new ResizeObserver(()=>{app.renderer.resize(host.value.clientWidth,host.value.clientHeight);updateCamera()});resizeObserver.observe(host.value)}
onMounted(setup)
