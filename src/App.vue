...
<template><div :class="['app-shell', themeClass]"><RouterView v-if="store.ready"/><div v-else class="boot"><span class="brand-mark">O</span><p>Charting the sky…</p></div><div v-if="needRefresh" class="agent-toast"><span class="agent-star">✦</span><div><strong>{{ $t('updateReady') }}</strong><p>Your local work is safe. Update when you are ready.</p></div><button @click="updateServiceWorker(true)">{{ $t('updateNow') }}</button></div><WebMcpActivity/></div></template>
